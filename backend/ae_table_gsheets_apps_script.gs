...
// shop_minus_meta is always shop - conv (arithmetically well-defined even
// when conv = 0). shop_vs_meta_pct is (shop - conv)/conv * 100 when conv > 0,
// otherwise 0 (rather than blank/null) so downstream sort + sum stay honest.
function enrichRows_(rows, assetMap, utmMap) {
  for (const r of rows) {
    ...
    r.shop_vs_meta_pct = conv > 0
      ? +(((shop - conv) / conv) * 100).toFixed(2)
      : 0;
